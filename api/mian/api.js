...
let sendemail_fun = function(username,data,result){
    ...
    let options = {  
        ...
        subject: data.tit, // 标题  
        //text: 'Hello world', // 文本  
        html: `<img src="http://193.112.102.203/public/system_pic/register_show_adv.jpg"><br><br><p>幸福的感觉</p><p>然后,旁若无人的痴然而笑</p>
        <p>---------------路小北</p>
        <a href="http://193.112.102.203`+data.url+`?regEmailMath=`+data.regEmailMath+`">
        点击链接，进行邮箱验证！http://193.112.102.203`+data.url+`?regEmailMath=`+data.regEmailMath+`</a>
        `
    };
...
const api = {
    ...
    "/upload/images":function(req,res){
        ...
        let form = new multiparty.Form({uploadDir:"./public/artwork_pic/"});
                    ...
                    if(_res){
                      //  User.updateOne(obj,{login_time:})
                        if(_res.passwd !== _result.passwd){
                            info.code = 1;
                            info.message = "密码错误，请重新输入";
                            res.end(JSON.stringify(info));
                        }else if(_res.passwd == _result.passwd){
                            if(_res.lock == 0){
                                info.code = 2;
                                info.message = "账号还未验证，请前往邮箱验证";
                                res.end(JSON.stringify(info));
                            }else if(_res.lock == 1){
                ...
                let reg =  reg_email.test(result.username);
                    if(reg){
                        if(result.passwd == ""){
                            ...
                            info.message = "密码不能为空";
                            res.end(JSON.stringify(info));
                        }else if(result.passwd !== result.repasswd){
                            info.code = 2;
